...
void setup()
{
  ...
  //test motor
  /**
  long testArr[2]={7000,8000};
  steppersControl.moveTo(gotoparking);
  steppersControl.runSpeedToPosition();
  delay(3000);
  steppersControl.moveTo(testArr);
  steppersControl.runSpeedToPosition();
  **/
...
void loop()
{
  executer();
/**
    steppersControl.moveTo(gototest); 
    steppersControl.runSpeedToPosition();
  
  if (testFlag=0){
    steppersControl.moveTo(gototest); 
    //stepperX.setSpeed(lowSpeed);
    //stepperY.setSpeed(lowSpeed);
    steppersControl.runSpeedToPosition();
  
    //steppersControl.moveTo(returntotest); 
    //stepperX.setSpeed(highSpeed);
    //stepperY.setSpeed(highSpeed);
    //steppersControl.runSpeedToPosition();
    testFlag=1;
  }
 **/
  ...
  if (commandFlag && order != previousOrder && failure==false){
    ...
    if (order.substring(0,1)=="J"){
      ...
      if (failure==false){
        ...
        stepperZ.moveTo(zSafety);//negatif //le seul qui va pas à un point particulier pour le moment// positif en faite mais cable bleu vers le truc vert 
        stepperZ.runToPosition();
